...
*   **架構決策記錄 (ADR):**
    *   *所有重大的架構決策都應被記錄。*
    *   **-> 參考: [架構決策記錄 (ADR)](./08_architecture_decision_record_template.md)**
...
*根據 [PRD (例如 02_project_brief_and_prd.md) ](../VibeCoding_Workflow_Templates/02_project_brief_and_prd.md) 中的使用者故事，定義最小可行產品 (MVP) 的範圍，並對功能模組進行優先級排序。*
...
*   **API 設計**:
    ...
    *   **-> 參考: [API 契約與接口定義](./04_api_contract_and_interface_definition.md)**
*   **資料模型 (Data Model)**:
...
*   **靜態結構分析**:
    *   **-> 參考: [檔案依賴關係](./03_file_dependencies_template.md)**
    *   **-> 參考: [模組規格與測試案例](./09_module_specification_and_tests.md)** 